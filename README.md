...
[Скачать MarkEdit для Windows (x64)](https://github.com/amoorkie/markedit/releases/latest/download/MarkEdit-Windows-1.33.1-win.5-Setup.exe)

Текущая версия: `1.33.1-win.5`.
...
- папка «Документы» используется как окружение по умолчанию;
- регистрация MarkEdit как приложения для `.md`, `.markdown`, `.mdown` и `.mkd`.

## Установка

1. Скачайте установщик по ссылке выше.
2. Запустите `MarkEdit-Windows-1.33.1-win.5-Setup.exe`.
